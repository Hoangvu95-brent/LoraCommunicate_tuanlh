...
SETUP_NODE = bytearray.fromhex("69 10 00 00 02 00 00 00 01")
form_resporn=["signature","opcode","status","from"]
RES_TOKEN = bytearray.fromhex("69 04 00 02")
RES_STATUS = bytearray.fromhex("69 11 01 02")
RES_STATUS_UPDATE = bytearray.fromhex("69 11 00 02")
RES_NODE_ONLINE   = bytearray.fromhex("69 02 00 02")
RES_SENSOR = bytearray.fromhex("69 00 00 00 02 00 00 00 01")
RES_FW_Update = bytearray.fromhex("69 01 00 00 02 00 00 00 01")
...
while True:
    ...
    '''if(data[0]==105 and data[1]==4 and data[2]==2 and data[3]==0): 
        ...
        ser.write(RES_TOKEN)
        ser.flush()'''
    if (data[0]==105 and data[1]==1 and data[4]==2 and data[8]==0):
        FW_Update()
        
    if(data[0]==105 and data[1]==6 and data[4]==2 and data[8]==0): #update magnetic sensor
        enableAlarm = data[7]
        timeOut     = int(data[5])
        timeOut     = timeOut<<8
        timeOut    += int(data[6])
        ...
        #ser.flush()
    #update cho specker
    if(data[0]==105 and data[1]==8 and data[4]==2 and data[8]==0): #update radio sensor
        speaker     = data[7]
        ...
        #ser.flush()
     #update cho RFID
    if(data[0]==105 and data[1]==10 and data[4]==2 and data[8]==0): #update RFID sensor
        pass
    
    if(data[0]==105 and data[1]==5  and data[4]==2 and data[2]==1 and data[8]==0):
        RES_SENSOR[1]=5
        ...
        ser.flush()
    if(data[0]==105 and data[1]==7  and data[4]==2 and data[2]==2 and data[8]==0):
        RES_SENSOR[1]=7
